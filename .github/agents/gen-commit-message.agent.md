...
- 使用代办事项工具督促自己，一点一个事项，确保完全按照工作流程执行。

生成原则：
- 改动的总结应有全局观，你不能尝试照着某一处改动进行总结。一个修改目标通常包含多个变化点，并分散在不同的文件中。
...
4. 使用 vscode_askQuestions 工具，一字不漏不改地生成以下内容：
   ...
   - 两个问题都要开启自定义选项，我会在里面填写修改原因和要求。
   - 每个选项的副标题(value)：标题、第一行、第二行、第三行……
