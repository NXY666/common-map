...
### 8.3 关于 `core <-> adapter` 的 `unknown`

#### 我的判断

应该改，但优先级低于 8.1 和 8.2。我同意问题方向，不同意把它排得比生命周期和边界更前。

#### 为什么我认为这条能说服你

`unknown` 在这里的坏处，主要是类型债，不是立即的运行时 bug。

说得更直白一点：

- `mount()` 没回滚，会把实例留在错误状态里，这是会炸的
- 对象边界不隔离，会让外部静默篡改内部状态，这也是会炸的
- `unknown` 更多是“IDE 不知道、实现层要断言、重构成本高”

所以这条该改，但它不是第一刀。

不过它也绝不是“可以永远不改”。因为一旦 adapter 从一个 demo 变成多个真实实现，`unknown -> as FooHandle -> as BarHandle` 这种模式会迅速扩散。到那时你不是在写抽象层，而是在各个实现里手工维护一张隐形类型表。

#### 简单修改示例

修改意图只有一句话：不要让 `unknown` 在核心接口里横着走，把句柄类型提升为 adapter 泛型。
...
export abstract class AbstractMapAdapter<
  THandles extends AdapterHandles,
> extends TypedEvented<AdapterEventMap> {
  public abstract createMap(...): THandles["map"];
...
class PseudoMapAdapter extends AbstractMapAdapter<PseudoHandles> {
  // 这里不需要再到处写 mapHandle as PseudoNativeMap
}
```

这个改法不会把 core 绑死到某个 SDK，它只是把“隐式约定”变成“显式类型”。
